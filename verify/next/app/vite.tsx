...
export const createDynamicEditorComponent = async () => {
    ...
    const defineWorkerLoaders: () => Partial<Record<string, WorkerLoader>> = () => {
        ...
        const defaultExtensionHostWorkerMain = () =>
            new workerFactory.Worker(new URL('@codingame/monaco-vscode-api/workers/extensionHost.worker', import.meta.url), { type: 'module' });
        const defaultTextMateWorker = () => new workerFactory.Worker(new URL('../bundle/textmateWorker/worker.js', import.meta.url), { type: 'module' });
