...
export const runClangdWrapper = async () => {
    ...
    const readiness = async () => {
        ...
        addFile(resourceUri);
        openFile(retrieveFilename(resourceUri));
...
const openFile = async (filename: string) => {
    const resourceUri = openFiles.get(filename);
...
const addFile = (resourceUri: vscode.Uri) => {
    ...
    openFilesSelect.onchange = (e: Event) => {
        ...
        console.log(`Selected file: ${newFilename}`);
        (window.MonacoEnvironment as MEEOPen).openFile(newFilename);
    };
